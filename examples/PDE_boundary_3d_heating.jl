...
function build_3D_heating_AB(dx, nx, dt)
    ...
    input_constant = dt / rho / specificHeat
...
function build_3D_PDE(N, nx, dx, dt, Tmax, Tstart; dense::Bool = true, implicit = false)
    ...
    Q  = 100. * Matrix(LinearAlgebra.I, ns, ns)
    Qf = 100. * Matrix(LinearAlgebra.I, ns, ns)./dt
    R  = 1.0 * Matrix(LinearAlgebra.I, nu, nu)./100000000
    ...
    if dense
        if implicit
            lqdm = DenseLQDynamicModel(s0, A, B, Q, R, N; Qf = Qf, sl = sl, su = su, ul = ul, uu = uu, K = K, S = S, implicit=implicit)
        else
            lqdm = DenseLQDynamicModel(s0, A, B, Q, R, N; Qf = Qf, sl = sl, su = su, ul = ul, uu = uu, K = K, S = S)
        end
    else
...
@time lqdm, d  = build_3D_PDE(N, nx, lenx / nx, dt, Tmax, Tstart; dense = true, implicit = false)
@time lqdms, d = build_3D_PDE(N, nx, lenx / nx, dt, Tmax, Tstart; dense = false)
