...
function build_QP_JuMP_model(Q,R,A,B, nt;
    ...
    @objective(model,Min,  sum( 1/2 * Q[s1, s2]*(x[s1,t])*(x[s2,t]) + cs[s1]*x[s1,t] for s1 in NS, s2 in NS, t in 1:(nt-1)) + 
            sum( 1/2 * R[u1,u2] * u[u1, t] * u[u2,t] + cu[u1]*u[u1,t] for t in 1:(nt-1) , u1 in NU, u2 in NU) + 
            sum(1/2 * Qf[s1,s2] * x[s1,nt] * x[s2, nt] + cs[s1]*x[s1,nt] for s1 in NS, s2 in NS))
