...
function build_H(
    ...
    H = SparseArrays.sparse([],[],Float64[],(ns*nt + nr*(nt)), (ns*nt + nr*(nt)))
...
function build_A(Ac,B, nt)
    ...
    A = SparseArrays.sparse([],[],Float64[],(ns*(nt-1)), (ns*nt + nr*nt))    
...
function get_QM(
    ...
    qp = QuadraticModels.QuadraticModel(c, H; A = A, lcon = len_con, ucon = len_con, lvar = lvar, uvar = uvar)
